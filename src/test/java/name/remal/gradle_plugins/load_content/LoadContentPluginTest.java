...
class LoadContentPluginTest {

    final Project project;

    @BeforeEach
    void beforeEach() {
        project.getPluginManager().apply(LoadContentPlugin.class);
    }

    @AfterEach
    void afterEach() throws Throwable {
        for (var registration : project.getGradle().getSharedServices().getRegistrations()) {
            if (registration.getName().contains(LoadContentPlugin.class.getPackageName())) {
    ...
    void pluginTasksDoNotHavePropertyProblems() {
        ...
        var taskClassNamePrefix = packageNameOf(LoadContentPlugin.class) + '.';
    ...
    @Test
    void test() {
        var extension = project.getExtensions().getByType(LoadContentExtension.class);
